...
const { requireAuth, checkPermission, checkAccesActivite } = require('../middlewares/auth');

router.use(requireAuth);

router.get('/', checkPermission('sous_activites', 'read'), sousActiviteController.getAll);
router.get('/:id', checkPermission('sous_activites', 'read'), sousActiviteController.getById);
